...
[rows, cols] = size(Data);
for r = 1:rows
    tempSumResults = SumResults
    for c = 1:cols
        for c2 = 1:cols
            divide = Data(r, c) / Data(r, c2);
            if(c ~= c2 & floor(divide) == divide)
                SumResults = SumResults + divide
                break;
            end
            if(SumResults > tempSumResults)
                break;
            end
        end
    end
